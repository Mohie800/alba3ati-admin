  ...
  useEffect(() => {
    async function fetch() {
      ...
      if (playerRes.status === "fulfilled") {
        setPlayer(playerRes.value.data.data.player);
        setGames(playerRes.value.data.data.games);
        setDeviceBanned(playerRes.value.data.data.deviceBanned);
      }
      if (friendsRes.status === "fulfilled") {
        setFriends(friendsRes.value.data.data.friends);
      }
      if (coinsRes.status === "fulfilled") {
        setCoinBalance(coinsRes.value.data.data.coins);
        setCoinTransactions(coinsRes.value.data.data.transactions);
      }
      if (shopRes.status === "fulfilled") {
        setFrameOptions(
          (shopRes.value.data.data.items as FrameOption[]).filter(
            (item) => item.type === "frame",
          ),
        );
      }
      if (nameHistoryRes.status === "fulfilled") {
        setNameHistory(nameHistoryRes.value.data.data.logs);
      }
    }
    fetch();
  }, [id]);
        ...
        <>
          ...
          <Card className="mb-6">
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Selecting a frame grants it to the player and equips it.
              </p>
            </CardHeader>
            <CardContent>
