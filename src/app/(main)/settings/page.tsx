...
function Toggle({
  ...
  return (
    <button
      ...
      className={cn(
        "relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50",
        checked
          ? tone === "destructive"
            ? "bg-destructive"
            : "bg-primary"
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!isValidUrl(playStoreUrl)) return setSaveError("Invalid Play Store URL");
    if (!isValidUrl(appStoreUrl)) return setSaveError("Invalid App Store URL");
    if (!isValidUrl(whatsappUrl)) return setSaveError("Invalid WhatsApp URL");
    if (!isValidUrl(telegramUrl)) return setSaveError("Invalid Telegram URL");
    if (!isValidUrl(discordUrl)) return setSaveError("Invalid Discord URL");
    ...
    try {
      const { data } = await api.put("/admin/app-settings", {
        ...
        maintenanceMode,
        maintenanceMessage,
        coinRewards: { gameComplete, gameWin, adReward, maxAdsPerDay },
        communityLinks: {
  ...
  return (
    <>
      <PageHeader
        title="App Settings"
        description="Force updates, maintenance mode, coin rewards, and community links."
      >
        {settings && (
        ...
        <div className="space-y-6 max-w-2xl">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1.5">
                    Users below the minimum version are forced to update.
                  </p>
                </div>
                <Toggle
                  checked={forceUpdate}
            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1.5">
                    Blocks creating or joining new rooms. Existing games and
                    reconnections are unaffected.
                  </p>
                </div>
                <Toggle
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground mt-1.5">
                How many coins players earn from different actions.
              </p>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    min={0}
                    value={gameComplete}
                    onChange={(e) => setGameComplete(Number(e.target.value))}
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    min={0}
                    value={maxAdsPerDay}
                    onChange={(e) => setMaxAdsPerDay(Number(e.target.value))}
                  />
                  <p className="text-xs text-muted-foreground mt-1">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Users below this version see the update screen
                </p>
              </div>
              <div>
                <label className="text-sm font-medium mb-1.5 block">
